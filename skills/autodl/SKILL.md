...
1. **每次开工先 `autodl status --json`**：余额、哪些实例 running（带卡计费=烧钱）、哪台是活动实例。只读免费。
2. **谁开的机谁关**：你启动的任务收尾后必须回到关机态（前台加 `--down`；后台完成后 `autodl down --yes`）。会话结束前 `status` 复核。
3. **别人的 running 实例绝不擅动**：账户里其它 running 可能是用户别的工作负载。发现了→提醒用户并给**针对单台**的命令 `autodl down --instance <uuid> --yes`。**不要建议 `stop-all`**——它会关掉所有 running，包括用户正在跑的东西；只有用户明确说"全部止损"才用。
...
6. 拿不准就 `--dry-run` 预览（run/down/stop-all/batch 都支持，不执行任何操作）。
...
- **`--json` 契约**：stdout 只有一行纯 JSON（进度在 stderr），直接 `| jq`。退出码：0 成功 / 3 余额不足 / 5 SSH 不可达 / 6 远端任务失败。
